...
- Auto-merge workflow gated on `pytest` + `claude-review`, opt-out via `critical` or `needs-manual-verification` labels on the PR or any linked issue. See `docs/agent-pr-workflow.md`.
- `.gitignore`
- `.env.example`
- `config/rpi-io.toml`
