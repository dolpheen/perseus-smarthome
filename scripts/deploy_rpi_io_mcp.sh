...
#   RPI_PROJECT_DIR    — remote project directory (default: /opt/raspberry-smarthome)
...
RPI_PROJECT_DIR="${RPI_PROJECT_DIR:-/opt/raspberry-smarthome}"
...
ssh "${SSH_OPTS[@]}" "${SSH_TARGET}" \
  "command -v uv >/dev/null 2>&1 || { echo \"ERROR: 'uv' not found on the Raspberry Pi. Install it first: curl -LsSf https://astral.sh/uv/install.sh | sh\"; exit 1; } && cd '${RPI_PROJECT_DIR}' && uv sync --no-dev"
